...
var CMD_CREATE_INDEX = "createSearchIndex";
var CMD_RELOAD_LOGBOOK = "reloadLogbook";
var CMD_NO_DEBUG = "NoDebug";
var CMD_NORMAL_DEBUG = "NormalDebug";
var CMD_HIGH_DEBUG = "HighDebug";
var CMD_HIGHEST_DEBUG = "HighestDebug";
...
/*********************************************************
* sends an ajax-request for with the password
*********************************************************/
function login()
{
    var pwd = $("pwd").value
    $("pwd").value = "";
    ajaxSendRequest("password"+pwd);
    return false;
}

/*********************************************************
* checks if the response is not a login failed message
*********************************************************/
function isLoginOk(node)
{
    if (node != null &&
        node.firstChild != null &&
        node.firstChild.nodeValue != "login failed")
    {
        $("passwordForm").hide();
...
function ajaxResponseHandle(data)
{
	...
	else
	{
        ...
        // check if user has logged in right
        if (isLoginOk(tempNode))
        {
...
function handleData(data, localActualNode)
{
    ...
    // happens when xml is malformed (e.g. bad conf.xml)
    //if (data.nodeValue!=null) alert(data.nodeValue);
...
function mkCommand(data, idPrefix)
{
	...
	if (code==CMD_CREATE_INDEX)
	{
		var elem = new Element("a");
		elem.writeAttribute("class","command");
		elem.insert(data.firstChild.nodeValue);		
		elem.writeAttribute("href","javascript:createSearchIndex('"+idPrefix+CMD_CREATE_INDEX+"')");
		return elem;
	}
	...
	if(code==CMD_NO_DEBUG)
 	{
		var elem = new Element("a");
		elem.writeAttribute("class","command");
		elem.insert(data.firstChild.nodeValue);
		elem.writeAttribute("href","javascript:setDebug('"+idPrefix+CMD_NO_DEBUG+"')");
		return elem;
	}
	if(code==CMD_NORMAL_DEBUG)
 	{
		var elem = new Element("a");
		elem.writeAttribute("class","command");
		elem.insert(data.firstChild.nodeValue);
		elem.writeAttribute("href","javascript:setDebug('"+idPrefix+CMD_NORMAL_DEBUG+"')");
		return elem;
	}
	if(code==CMD_HIGH_DEBUG)
 	{
		var elem = new Element("a");
		elem.writeAttribute("class","command");
		elem.insert(data.firstChild.nodeValue);
		elem.writeAttribute("href","javascript:setDebug('"+idPrefix+CMD_HIGH_DEBUG+"')");
		return elem;
	}
	if(code==CMD_HIGHEST_DEBUG)
 	{
		var elem = new Element("a");
		elem.writeAttribute("class","command");
		elem.insert(data.firstChild.nodeValue);
		elem.writeAttribute("href","javascript:setDebug('"+idPrefix+CMD_HIGHEST_DEBUG+"')");
		return elem;
	}
...
function showInfo(content)
{
	...
	else
	{
		...
		//elem.onclick = function() { hideInfo()};
		// create close hint
...
* refill the deleted elements
*********************************************************/
function createSearchIndex(command)
{
	if(!isAjaxOccupied)
	{
		var parentId = command.substr(0,command.lastIndexOf("/"));
...
/*********************************************************
* sets a debug level for Search Server Logging 
*********************************************************/
function setDebug(command)
{
	if(!isAjaxOccupied)
	{
		var parentId = command.substr(0,command.lastIndexOf("/"));
