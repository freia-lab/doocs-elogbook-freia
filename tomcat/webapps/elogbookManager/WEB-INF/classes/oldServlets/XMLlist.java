...
public class XMLlist extends HttpServlet {
	...
	String res;
        //File webapps = new File(context_.getRealPath("")).getParentFile();
        //File webapps = new File("/home/jstrampe/tomcat5.5/webapps");
        //File webapps = new File("/var/www");
        //return webapps.getAbsolutePath()+file;
	if (file.startsWith("/~")) {
		...
		xmlout = xmlout.append("<xml_file>" + fullname + "</xml_file>");
	    	if (file.isDirectory()) {
                    ...
                    for (int i=0; i<filenames.length; i++) {
                        ...
                        if (afile.canRead() && !filenames[i].toString().startsWith(".") && filetypes[i] == UNASSIGNED)  {
                            ...
                            {
                                rawfilename = aname.substring(0, aname.lastIndexOf("."));
                            }*/
            ...
            */
            ...
            if (xsl_param == null || xsl_param.length() < 4) {
                ...
                if (xslfile.exists()) {
                    ...
                    out.println("<p>Error in XSL file</p></BODY></HTML>");
                    out.close();
                }
            }
	} catch	(Exception ioe) {
 	    ...
 	    out.println("<HTML><HEAD><TITLE>Error Page</TITLE></HEAD><BODY>");
  	    ...
  	    out.println("<p>Error at: " + debug_sts + "</p>");
            ...
            out.println("<p>xml-file: "+ bugout +"</p></BODY></HTML>");
            out.close();
	}
	//out.close();
    }
