...
/**
 * subclass to handle different eLogbook instances
 * 
...
public class Tree extends DataComponent {
    ...
    //private long vorher=0;
    //private long nachher=0;
    ...
    // Does the user want to add an empty folder ?
    ...
    /**
     * bei der ersten initialisierung wird der Logbuchname gesetzt
     * und die XML Baumdaten gelsen.
     *
     * @return ob XML daten geladen werden konnten
     */
    ...
    {   
        ...
        if (getParameter("fill",request)!=null)
        {
            ...
            {
                LogHelper.getInstance().log(ex.toString()+" Tree.java 94 while filling");
        ...
        if (getParameter("add",request)!=null)
        {
            ...
            {
                LogHelper.getInstance().log(ex.toString()+" Tree.java 116 while adding");
            ...
            getParameter("p2",request)!=null)
        {
            ...
            {
                String debugMsg = " Tree.java 151 p1 = "+p1+" p2 = "+ p2
                                + "\nname = " + this.name
                                + "\npath "+ this.dataPath
                                + "\n-----------";
                LogHelper.getInstance().log(ex.toString()+debugMsg);
    ...
    {
        /*String result = "Tree "+this.name+"\n";
        if(doc==null) return result.concat("no data is loaded");
        int anzSub = doc.getElementsByTagName("S").getLength();
        int anzData = doc.getElementsByTagName("D").getLength();
        result += "the tree has:\n"+ anzSub +" subfolders\n"+anzData+" data entries\n\n\n";
        result += wholeTreeToString(doc.getDocumentElement());*/
    ...
    private void ladeDaten()
    {
        try
        {
            ...
            String path = "";
            //path = getServletContext().getRealPath("/WEB-INF/classes/newXML.xml");
            //path = "/home/jstrampe/Desktop/javascript Tree/pfaddaten.xml";
            //path = "/home/jstrampe/Desktop/javascript Tree/neuXML.xml";
            //path = "/var/www/TESTelog/jsp/treeData.xml";
            //path = "/var/lib/tomcat5/webapps/"+TESTelog+"/treeData.xml";
            //path = "/var/lib/tomcat5/webapps/"+name+"/treeData.xml";
            path = dataPath + "/treeData.xml";
            //path.replace("//","/");
            //path = "http://mcsstrampe/TESTelog/jsp/treeData.xml";

            // should be the path to the elog folder
            //path = getServletContext().getRealPath("");
            //path = path.substring(0,path.length()-4);
            //path += "TESTelog/treeData.xml";
        ...
        {
            LogHelper.getInstance().log(ex.toString()+" Tree.java 233 while loading "+dataPath+"/treeData.xml");
