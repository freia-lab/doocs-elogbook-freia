...
// picture paths
var _iconFolder 	= "/folder.png";
var _iconFolderOpen	= "/folder_open.png";
var _iconFile 		= "/file.png";
var _iconFileOpen 	= "/file_open.png";
var _iconSpacer 	= "/spacer.png";
// picture size
var _picSize = "20";
// servlet path
...
var _jumpPos = "";
// indicates if it is the first load should be blocked
var _blockFirstLoad = false;
...
* initialization
*********************************************************/
function init(host,logroot,imagedir,viewServlet,viewXsl,actDir,dataPath)
{
	// calculate the jump to position for show.jsp
	var showIndex = window.parent.location.href.indexOf("show.jsp");
	var posIndex = window.parent.location.href.indexOf("&pos=");
	if(showIndex<posIndex) 
	{
		actDir = window.parent.location.href.substring(showIndex+13,posIndex);
		_jumpPos = window.parent.location.href.substring(posIndex+5,window.parent.location.href.length);
	}
	
	// block first load if list_frame contains a legal site on first load
	_blockFirstLoad = (window.parent.frames[_contentFrame].location.href.indexOf("XMLlist?")!=-1);
	
	//alert("host "+host+"\n"+"logroot "+logroot+"\n"+"imagedir "+imagedir+"\n"+"viewServlet "+viewServlet+"\n");
	_iconFolder 	= host + imagedir + _iconFolder;
	_iconFolderOpen	= host + imagedir + _iconFolderOpen;
	_iconFile 		= host + imagedir + _iconFile;
	_iconFileOpen 	= host + imagedir + _iconFileOpen;
	_iconSpacer		= host + imagedir + _iconSpacer;
	_servletPath 	= host + _servletPath;
	_xmlList1 		= host + viewServlet + _xmlList1;
	_xmlList2 		= "&xsl=" + viewXsl + _xmlList2;
	_elogbookName	= logroot.substring(1, logroot.length);
	_dataPath		= logroot + dataPath;
...
function showInRightFrameData(dataId)
{
	...
	else
	{
		...
		if (parentFrame!=null)
		{
			var file = _dataPath +"/"+ dataId;
			var path = _xmlList1 + file + _xmlList2;
...
function showAddress(address)
{
	...
	sub = sub.substring(_dataPath.length+1,sub.length);
	showElement(sub);
}
